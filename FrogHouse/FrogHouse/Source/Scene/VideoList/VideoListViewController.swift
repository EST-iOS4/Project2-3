    ...
    private func updateMenu(categories: [String], selectedIndex: Int) {
        let actions = categories.enumerated().map { (index, title) in
            UIAction(
                ...
                Task {
                    do {
                        try await self?.viewModel.selectCategory(at: index)
                    } catch {
...
extension VideoListViewController {
    private func setupDataSource() {
        dataSource = UICollectionViewDiffableDataSource<Section, VideoListItem>(
            collectionView: videoCollectionView
        ) { collectionView, indexPath, videoItem in
            let cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: VideoCell.reuseIdentifier,
                for: indexPath
            ) as? VideoCell ?? VideoCell()
